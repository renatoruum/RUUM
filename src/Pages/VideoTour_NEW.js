...
const VideoTour = () => {
    ...
    const handleGenerateScript = async () => {
        ...
        try {
            ...
            // Atualizar promptText automaticamente
            setRunwayForm(prev => ({
                ...prev,
                promptText: `Cinematic shot of ${script.split(' ').slice(0, 5).join(' ')}...`
            }));
    ...
    const handleTextToSpeech = async () => {
        ...
        try {
            ...
            console.log('URL completa:', `${API_CONFIG.BASE_URL}/api/elevenlabs/text-to-speech`);
            console.log('Timestamp:', new Date().toISOString());
            console.log('============================');

            // Usar apiCall especial para TTS que pode retornar blob
            ...
            if (!audioResponse.ok) {
                ...
                console.error('❌ Erro na resposta TTS:', {
                    status: audioResponse.status,
                    statusText: audioResponse.statusText,
                    headers: Object.fromEntries(audioResponse.headers),
                    errorText: errorText
                });
            ...
            console.log('📋 Headers completos da resposta:', Object.fromEntries(audioResponse.headers));
            console.log('📋 Status da resposta:', audioResponse.status, audioResponse.statusText);
            console.log('📋 URL da requisição:', audioResponse.url);
            
            if (contentType && contentType.includes('application/json')) {
                // Resposta JSON com URL
                const jsonResponse = await audioResponse.json();
                console.log('✅ TTS bem-sucedido! (JSON)');
                console.log('📋 Resposta completa do áudio:', jsonResponse);
                console.log('📋 Estrutura da resposta:', JSON.stringify(jsonResponse, null, 2));
                
                const publicAudioUrl = jsonResponse.url || jsonResponse.data?.url || jsonResponse.audioUrl;
                
                if (!publicAudioUrl) {
                    console.error('❌ Nenhuma URL encontrada na resposta:', jsonResponse);
                    throw new Error('URL pública do áudio não foi retornada');
                }
                
                console.log('🔗 URL PÚBLICA DO ÁUDIO (para teste no Postman):', publicAudioUrl);
                console.log('🔗 Copie esta URL para testar no Postman:', publicAudioUrl);
                ...
                console.log('🔗 BLOB URL LOCAL (não funciona no Postman):', audioBlobUrl);
                console.log('ℹ️ Para testar no Postman, você precisa da URL pública do servidor');
    ...
    const handleTestTTS = async () => {
        ...
        try {
            ...
            if (!audioResponse.ok) {
                const errorText = await audioResponse.text();
                console.error('❌ Erro na resposta TTS (teste):', {
                    status: audioResponse.status,
                    statusText: audioResponse.statusText,
                    headers: Object.fromEntries(audioResponse.headers),
                    errorText: errorText
            ...
            // Verificar se é JSON ou blob
            const contentType = audioResponse.headers.get('content-type');
            console.log('📋 Content-Type da resposta (teste):', contentType);
            console.log('📋 Headers completos da resposta (teste):', Object.fromEntries(audioResponse.headers));
            console.log('📋 Status da resposta (teste):', audioResponse.status, audioResponse.statusText);
            
            if (contentType && contentType.includes('application/json')) {
                // Resposta JSON com URL
                const jsonResponse = await audioResponse.json();
                console.log('✅ Teste TTS bem-sucedido! (JSON)');
                console.log('📋 Resposta completa do áudio (teste):', jsonResponse);
                console.log('📋 Estrutura da resposta (teste):', JSON.stringify(jsonResponse, null, 2));
                
                const publicAudioUrl = jsonResponse.url || jsonResponse.data?.url || jsonResponse.audioUrl;
                
                if (!publicAudioUrl) {
                    console.error('❌ Nenhuma URL encontrada na resposta (teste):', jsonResponse);
                    throw new Error('URL pública do áudio não foi retornada');
                }
                
                console.log('🔗 URL PÚBLICA DO ÁUDIO TESTE (para Postman):', publicAudioUrl);
                console.log('🔗 Copie esta URL para testar no Postman:', publicAudioUrl);
                ...
                console.log('📋 Tamanho do áudio (teste):', audioBlob.size, 'bytes');
                console.log('📋 Tipo do blob (teste):', audioBlob.type);
                
                const audioBlobUrl = URL.createObjectURL(audioBlob);
                console.log('🔗 BLOB URL LOCAL TESTE (não funciona no Postman):', audioBlobUrl);
                console.log('ℹ️ Para testar no Postman, você precisa da URL pública do servidor');
    ...
    const handleGenerateVideo = async () => {
        ...
        try {
            ...
            console.log('✅ Vídeo gerado:', response);
                ...
                <div className="card-body">
                    ...
                    <div className="mt-3">
                        <div className="alert alert-success small">
                            ...
                            <strong>🚀 Mais rápido:</strong> Menos pontos de falha e processamento mais eficiente.
                        </div>
                    </div>
                </div>
            </div>
